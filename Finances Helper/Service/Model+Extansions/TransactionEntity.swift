...
//  Finances Helper
//  Created by Kendrick  on 10/06/24.

// Extension for TransactionEntity providing additional functionality
import Foundation
import CoreData
import SwiftUI
...
enum TransactionTimeFilter: CaseIterable, Identifiable, Equatable {
    ...
    var date: (start: Date?, end: Date?) {
        ...
        case .select(let start, let end): return (start.noon, end.noon)
        }
    }
    ...
    var navTitle: String? {
        ...
        case .select(let start, let end):
            return "\(start.toStrDate(format: "MMM d")) - \(end.toStrDate(format: "MMM d"))"
        }
    }
}
